...
def plot_passive_evolution(good_traces, passive_sizes, selected_logits, plot_path):
  ...
  ax1.fill_between(steps, 0, passive_sizes, alpha=0.15, color='gray', label='passive size')
  ...
  for ci, cl_idx in enumerate(all_clauses):
    ...
    for si, (ss, sp) in enumerate(segments):
      ax1.plot(ss, sp, color=color, linewidth=0.8, alpha=0.7,
  ...
  ax2.plot(steps, selected_logits, color='red', linewidth=1.0, alpha=0.8, label='selected logit')
  ax2.set_ylabel('logit of selected clause', color='red')
...
if __name__ == "__main__":
  ...
  # load model
  (_loop, model_state_dict, _optimizer_state_dict) = torch.load(model_path, weights_only=False)
