...
def precompute_gweight_indices(gweight_terms, gweight_clauses, cl_nums_ordered):
  """Build index tensors for vectorized gweight tree processing (HOL version).

  Global index scheme: 0 = zero embedding, 1 = term var, 2 = sort var, 3.. = terms.
  HOL terms always have exactly 4 args, so no multi_fix needed.
  """
  term_to_global = {0: 1, 1: 2}  # term id 0 (term var) -> 1, term id 1 (sort var) -> 2
  next_idx = 3

  # Topological sort terms into layers
  term_layers_map = {}
  raw_layers = []
  ...
  for id, functor, sign, args in gweight_terms:
    layer_idx = 0
    for a in args:
      if a > 1:  # both 0 and 1 are variable slots
  ...
  # Assign global indices in layer order (must match torch.cat order)
  for raw_layer in raw_layers:
  ...
  # Build per-layer index tensors — 4 fixed args, no multi_fix
  term_layers = []
  for raw_layer in raw_layers:
    K = len(raw_layer)
    functor_idxs = torch.empty(K, dtype=torch.long)
    sign_vals = torch.empty(K)
    arg0_idxs = torch.zeros(K, dtype=torch.long)
    arg1_idxs = torch.zeros(K, dtype=torch.long)
    arg2_idxs = torch.zeros(K, dtype=torch.long)
    arg3_idxs = torch.zeros(K, dtype=torch.long)

    for j, (id, functor, sign, args) in enumerate(raw_layer):
      assert len(args) == 4
      functor_idxs[j] = functor
      sign_vals[j] = sign
      arg0_idxs[j] = term_to_global[args[0]]
      arg1_idxs[j] = term_to_global[args[1]]
      arg2_idxs[j] = term_to_global[args[2]]
      arg3_idxs[j] = term_to_global[args[3]]
...
def run_vectorized_gweight(gweight_symbol_embeds, gweight_var_embed, gweight_svar_embed,
                           gweight_data, gweight_term_combine, gweight_static_tweak):
  ...
  svar_embed = gweight_svar_embed(torch.tensor(0.0)).unsqueeze(0)
  all_term_embeds = torch.cat([zero_embed, var_embed, svar_embed], dim=0)

  for functor_idxs, sign_vals, arg0_idxs, arg1_idxs, arg2_idxs, arg3_idxs in term_layers:
    functor_e = gweight_symbol_embeds[functor_idxs]
    signs = sign_vals.unsqueeze(1)
    arg0_e = all_term_embeds[arg0_idxs]
    arg1_e = all_term_embeds[arg1_idxs]
    arg2_e = all_term_embeds[arg2_idxs]
    arg3_e = all_term_embeds[arg3_idxs]

    res = gweight_term_combine(torch.cat([functor_e, signs, arg0_e, arg1_e, arg2_e, arg3_e], dim=1))
...
class LearningModel(torch.nn.Module):
  ...
  def pre_forward(self):
    ...
    gweight_features = run_vectorized_gweight(
      gweight_symbol_embeds, self.nn.gweight_var_embed, self.nn.gweight_svar_embed,
      gweight_data,
  ...
  def _old_pre_forward_for_verify(self, initial_clause_gage, gweight_symbol_embeds):
    """Run the old dict-based GAGE/GWEIGHT path for verification.
    ...
    self.nn.gweight_term_embed_store[1] = self.nn.gweight_svar_embed.forward(torch.tensor(0.0))
