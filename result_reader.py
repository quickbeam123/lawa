...
SELECTION = ["Problems/SEU/SEU076+1.p",
  "Problems/SWB/SWB013+2.p",
  "Problems/SWV/SWV814-1.p",
  "Problems/SEV/SEV580+1.p",
  "Problems/NUN/NUN077+2.p",
  "Problems/GRP/GRP170-2.p",
  "Problems/LCL/LCL462+1.p",
  "Problems/GRP/GRP667-4.p",
  "Problems/CSR/CSR118+2.p",
  "Problems/RNG/RNG006-10.p",
  "Problems/SWC/SWC401+1.p",
  "Problems/LCL/LCL439-1.p",
  "Problems/KLE/KLE160+1.p",
  "Problems/HAL/HAL001+2.p",
  "Problems/SWC/SWC412+1.p",
  "Problems/SET/SET176-6.p",
  "Problems/SWC/SWC324-1.p",
  "Problems/REL/REL024-2.p",
  "Problems/REL/REL009+1.p",
  "Problems/SEU/SEU323+2.p",
  "Problems/CSR/CSR116+16.p",
  "Problems/SWV/SWV293-1.p",
  "Problems/SEU/SEU018+1.p",
  "Problems/CSR/CSR114+28.p",
  "Problems/ITP/ITP208_1.p",
  "Problems/LAT/LAT323+2.p",
  "Problems/SET/SET601+3.p",
  "Problems/COM/COM124+1.p",
  "Problems/GRP/GRP423-1.p",
  "Problems/SEU/SEU157+2.p",
  "Problems/CSR/CSR116+5.p",
  "Problems/SWC/SWC117-1.p",
  "Problems/SWW/SWW265+1.p",
  "Problems/AGT/AGT005+1.p",
  "Problems/FLD/FLD020-1.p",
  "Problems/NUM/NUM030-1.p",
  "Problems/SWW/SWW364+1.p",
  "Problems/SWV/SWV332-1.p",
  "Problems/ANA/ANA016-1.p",
]

if __name__ == "__main__":
  # simple mode - extracts all successes from a particular exper
  if False:
  ...
  # a sweep mode; show me the new problems each iter
  if True:
    # ./result_reader.py ~/jar2026/seed42_nd

    exper_path = sys.argv[1]

    MISSION = "train"

    covered = set()

    selection_info = {prob : [] for prob in SELECTION}

    for iter in range(1,26):
      cur_loop_path = os.path.join(exper_path,f"loop{iter}")
      print(cur_loop_path)

      res_file_path = os.path.join(cur_loop_path,f"{MISSION}_res.pt")
      (_meta,results) = torch.load(res_file_path,weights_only=False)

      for prob,runs in results.items():
        if prob in selection_info:
          for (i,ilim,info) in runs:
            if i == 0 and info.status == "uns":
              selection_info[prob].append(info.activations)
              break
          else:
            selection_info[prob].append(None)

        for (i,ilim,info) in runs:
          if info.status == "uns":

            if prob not in covered and iter == 2:
              if i == 0: # newly covered, without noise
                print("  ",prob,info)

            covered.add(prob)

    print()
    for prob, his_story in selection_info.items():
      print(prob, his_story)
